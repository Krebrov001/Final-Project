...
if (my_rank == 0):
    ...
    grid, grid_size, num_generations, output_generations = read_in_file(comm, filename, comm_sz)
...
# Scatter() should be used for NumPy arrays vs scatter()
# The chunk size and the data type of elements are inferred by the Python interpreter.
comm.Scatter(sendbuf=grid, recvbuf=chunk, root=0)
...
for generation in range(num_generations):
    ...
    for row in range(num_rows):
        ...
        elif (my_rank == comm_sz-1):  # bottom rank
            # determine what the row above should be
            if (row == 0):
    ...
    if ((generation + 1) % output_generations == 0):
        # comm.gather() in Python returns the gathered object.
        # It returns the result into rank root=0 only, it returns None into the other ranks.
        # It returns a Python list of 2D numpy arrays, each one is a separate chunk gotten from a rank.
        grid = comm.gather(sendobj=current_chunk, root=0)
        
        if (my_rank == 0):
            # Get the first (top) 2D numpy array.
            new_grid = grid[0]
            # Append the rest of the 2D numpy arrays to the first (top) one, row-wise.
            for i in range (1, comm_sz):
                new_grid = np.concatenate((new_grid, grid[i]), 0)
            
            # Reassign the grid, replace the Python list of 2D numpy arrays with a single 2D numpy array
            # containing the whole entire matrix.
            grid = new_grid
