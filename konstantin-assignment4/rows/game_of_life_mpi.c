...
int main(int argc, char** argv) {
    ...
    if (my_rank == 0) {
        ...
        grid = read_in_file(filename, &grid_size, &num_generations, &output_generations, comm_sz);
    ...
    if (my_rank == 0) {
        ...
        size_t input_string_len = strlen(argv[1]);
        ...
        while (isdigit(argv[1][offset-1])) {
            --offset;
        }
        strcpy(output_filename + offset + 1, argv[1] + offset);
    ...
    if (my_rank == 0) {
        ...
        strcpy(timing_filename + offset + 1, argv[1] + offset);
    ...
    for (int generation = 0; generation < num_generations; ++generation) {
        ...
        if ((generation + 1) % output_generations == 0) {
            MPI_Gather(current_chunk, chunk_size, MPI_UINT8_T, grid, chunk_size, MPI_UINT8_T, 0, MPI_COMM_WORLD);
